...
- `assets/scene.js` — the three.js particle scene. Decorative only: pages
  ...
  `velora:voice` event the hero demo dispatches and swells while "listening".
- `styles.css` — every colour is an oklch token in `:root`; the dark palette
  is written twice (media query and `[data-theme]`) and the site test keeps
  the two identical. Brand values come from `docs/DESIGN.md`.
- `assets/velora-mark.svg`, `assets/app-icon.png`, `assets/og.png` — brand
  assets generated by `scripts/make-icon.py` (`uv run --script
  scripts/make-icon.py`). Never edit them by hand; see `docs/DESIGN.md`.
