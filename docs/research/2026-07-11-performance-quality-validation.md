...
The optimization kept the production models and precision unchanged:

- Speech: `mlx-community/whisper-large-v3-turbo`
- Cleanup: `mlx-community/Qwen3.5-4B-MLX-8bit`

No model-size, quantization, output-token, or quality reduction was used to obtain the results below.
...
All cases used the production Qwen model above, temperature zero, and a prepared-prefix cache hit. Prefix preparation took 0.95–1.06 seconds during recording; it is excluded from stop-side generation because that work is deliberately moved off the release path.

| Case | Words | Stop-side cleanup | Result check |
|---|---:|---:|---|
| Declarative | 13 | 412 ms | final full stop present |
| Grammar repair | 18 | 468 ms | agreement/tense repaired conservatively |
| Question | 18 | 444 ms | question mark present |
| Terminal prose | 23 | 543 ms | final full stop retained |
| Names/numbers | 21 | 629 ms | names, Q3, time, and currency retained |
| Long prose | 53 | 1,075 ms | meaning and details retained |
