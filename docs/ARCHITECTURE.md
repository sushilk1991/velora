...
2. An audio-only Core Audio process tap writes computer audio (`Them`) as a separate crash-resilient CAF track. It never requests display or window frames. Capture is not declared healthy until both requested tracks deliver frames, and microphone frames count only when a sample is louder than one 16-bit step of full scale in any accepted PCM layout (the same silence line the engine uses). A microphone that delivers only silence (a closed MacBook lid, a muted interface) starts the meeting degraded after 5 s and clears the flag at its first sound; a microphone whose writes or stream fail during startup fails the start instead. Only a microphone that never delivered sound gets the error sound and HUD warning, once per meeting; zeros after real sound only mark the meeting. Both are logged with the device, format and lid state. If computer audio fails, the persistent HUD says `Mic only` and keeps Stop available, and the audio recorded before the failure is kept.
...
4. `MeetingProcessor` transcribes each speaker track in bounded time chunks, checkpoints completed chunks, and resumes interrupted work. A track the app cannot open, or the engine rejects as `unsupported_audio` (format or decode), is skipped and recorded as a track issue. A file the engine could not read this time (`audio_load_failed`: converter timeout, full disk, a file that changed or vanished while it was read) is retried three times, at least 30 s apart, then fails the job; it is never skipped. Any other engine failure fails the job for Retry. A track of digital silence skips speech recognition and is labelled `silent`; one under 0.2 s is labelled `too_short`. The meeting fails only when no track yields speech. It then runs a chunked map/reduce notes pass for summary, decisions, and action items and keeps the notes (flagged partial) when some sections fail; after three sections in a row whose first attempt timed out (even if a split retry recovered them), each later section gets one attempt with no split retry; an output-limit or malformed first attempt costs no timeout and does not count. When the notes model itself goes away the engine restarts and the app requeues the job, so a fragment never ships as final notes. Notes that failed get one automatic retry, claimed in SQLite before it is queued (at most three per engine start), never repeated, and never applied to a cancelled job. Notes SQLite refuses to save are written again after 1 s, then generated once more.
...
Stop consumes only a completed glossary; it never waits. Stop, cancel, errors, and new sessions invalidate late readers. Secure input, denied AX permission, a different WindowServer window, and a changed focused window fail closed; a window the user merely moved or resized does not. External listening and Action recording remain excluded. Context is neither logged nor stored in history/dictionary state or shared model-prefix caches. Screen context has no in-app off switch; old opt-outs are ignored. Accessibility reads continue without Screen Recording permission, which gates only the local OCR fallback. The optional Screen Recording grant is requested only through Settings > Dictation > Allow Local Screen Text Recognition. Pin-stage refusals always log their reason code, and the wait when the reader polled; `VELORA_CONTEXT_DIAGNOSTICS=1` adds the remaining stage and refusal metadata (counts and reasons, never screen text). Details and paired validation: `docs/plans/2026-09-16-context-glossary-design.md`.
