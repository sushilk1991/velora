...
site wordmark is set in — closed by an apricot full stop: "Say it once. It's
already typed." The V is white on a sky plate (deep sky top-left → sky
bottom-right) with a soft lighter-sky bloom low-left; the dot is the brand's
warm accent (`--accent-2` on the site).

    ┌──────────────────────────┐
    │  plate: deep sky → sky   │   1024 canvas
    │        ╲   ╱             │   824 squircle (Apple's macOS grid)
    │         ╲ ╱   ● apricot  │   V cap height 640
    │   bloom  V               │
    └──────────────────────────┘
...
# Palette (brand v2) — keep in step with docs/DESIGN.md, HUDStyle.swift and
# site/styles.css. The plate runs top-left → bottom-right through the three
# sky stops; the bloom is a lighter sky, not a second hue.
PLATE_TOP = "#0b5ea0"
PLATE_MID = "#0f74c5"
PLATE_BOTTOM = "#4dacf6"
BLOOM = "#8ccdff"
...
# Warm ink: the OG card's ground and the site's dark surfaces.
INK_WINDOW = "#161311"
INK_CARD = "#201d1b"
PAPER = "#faf8f5"
PAPER_MUTED = "#c9c2ba"
...
def mark_svg(*, full_bleed: bool, effects: bool, view_box: str | None = None) -> str:
  ...
  <defs>
    ...
    <radialGradient id="bloom" cx="0.16" cy="1.02" r="0.72">
      <stop offset="0" stop-color="{BLOOM}" stop-opacity="0.55"/>
      <stop offset="0.45" stop-color="{BLOOM_EDGE}" stop-opacity="0.22"/>
  ...
  <g{shadow}>
    ...
    <circle cx="{dot_x:.1f}" cy="{dot_y:.1f}" r="{DOT_RADIUS}" fill="{APRICOT}"/>
  </g>
</svg>
"""
...
def og_svg() -> str:
    ...
    display face, on the site's warm-ink ground."""
  ...
  <defs>
    <radialGradient id="ground" cx="0.18" cy="0.5" r="0.9">
      <stop offset="0" stop-color="{INK_CARD}"/>
      <stop offset="1" stop-color="{INK_WINDOW}"/>
    </radialGradient>
  ...
  <g transform="translate({text_x + 8} 392) scale({scale_tag} -{scale_tag})" fill="{PAPER_MUTED}">{tagline}</g>
  <g transform="translate({text_x + 8} 452)" fill="{PAPER_FAINT}" font-family="-apple-system, Helvetica Neue, Helvetica, Arial, sans-serif" font-size="22" letter-spacing="0.08">
