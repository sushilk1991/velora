...
grep -Fq 'test -f "$ENGINE/src/velora_engine/cleanup_ipc.py"' scripts/verify-dmg.sh
...
grep -Fq 'if [[ "${VELORA_DISTRIBUTION:-0}" != "1" ]]; then' scripts/make-app.sh
grep -Fq "Print :VeloraEngineDir" scripts/verify-dmg.sh
